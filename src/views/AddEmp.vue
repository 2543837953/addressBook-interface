  ...
  <div class="add-emp">
    ...
    <div class="layout" >
      ...
      <div class="submit-btn">
        <input type="submit" id="submit" value="添加" @click="addEmp()">
      </div>
    </div>
...
export default {
  ...
  methods:{
    addEmp(){
      ...
      fetch(this.$vars.url + 'employees/list/new', {method: 'post', headers: header,body:JSON.stringify(data)})
          .then(res => {
            switch (res.status){
              case 200:
                this.stateCode(true,"创建人员成功","EmployeeList")
                break;
              case 422:
                res.json().then(d=>{
